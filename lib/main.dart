import 'package:flutter/material.dart';
import 'package:flutter_catalog/core/store.dart';
import 'package:flutter_catalog/screens/cart_page.dart';
import 'package:flutter_catalog/screens/dynamic_link.dart';
import 'package:flutter_catalog/screens/login_page.dart';
import 'package:flutter_catalog/screens/navigation_context.dart';
import 'package:flutter_catalog/utilis/routes.dart';
import 'package:velocity_x/velocity_x.dart';
import 'models/dynamicLinkHelper.dart';
import 'screens/home_page.dart';
import 'widgets/themes.dart';

void main() {
  runApp(VxState(store: MyStore(), child: MyApp()));
}

class MyApp extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    DynamicLinkHelper.retrieveDynamicLink(context);

    return MaterialApp(
      themeMode: ThemeMode.system,
      theme: MyTheme.lightTheme(context),
      darkTheme: MyTheme.darkTheme(context),
      debugShowCheckedModeBanner: false,
      initialRoute: MyRoutes.dynamicRoute,
      navigatorKey: NavigationService.navigatorKey,
      routes: {
        // "/": (context) => DynamicLink(),
        MyRoutes.homeRoute: (context) => HomePage(),
        MyRoutes.loginRoute: (context) => LoginPage(),
        MyRoutes.cartRoute: (context) => CartPage(),
        MyRoutes.dynamicRoute: (context) => DynamicLink()
      },
    );
  }
}
